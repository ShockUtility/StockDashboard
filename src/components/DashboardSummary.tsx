    ...
    <section className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div className="summary-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%)', padding: '24px', borderRadius: '24px', border: '1px solid rgba(59, 130, 246, 0.2)', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.3)' }}>
          ...
          <strong style={{ fontSize: '2rem', letterSpacing: '-1px', color: '#fff' }}>{formatMoney(totalCurrentValueKRW, 'KRW')}</strong>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', padding: '24px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <strong className={totalReturnAmountKRW >= 0 ? 'text-success' : 'text-danger'} style={{ fontSize: '1.6rem', letterSpacing: '-0.5px' }}>
      ...
      <div className="cash-details" style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', marginTop: '8px', borderTop: '1px solid var(--glass-border)', paddingTop: '20px' }}>
        <div style={{ flex: 1 }}>
          ...
          <strong style={{ fontSize: '1.2rem', color: 'var(--text-primary)' }}>{formatMoney(totalKRWAssets, 'KRW')}</strong>
        </div>
        <div style={{ flex: 1, borderLeft: '1px solid var(--glass-border)', paddingLeft: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <strong style={{ fontSize: '1.2rem', color: 'var(--text-primary)' }}>{formatMoney(totalUSDAssets, 'USD')}</strong>
            <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '2px' }}>≈ {formatMoney(totalUSDAssets * exchangeRate, 'KRW')}</span>
          </div>
        </div>
        <div style={{ flex: 1, borderLeft: '1px solid var(--glass-border)', paddingLeft: '16px' }}>
          <div className="text-secondary" style={{ fontSize: '0.8rem', marginBottom: '4px', color: '#f59e0b', fontWeight: 600 }}>총 보유 현금</div>
          <strong style={{ fontSize: '1.3rem', color: '#f59e0b' }}>{formatMoney(totalKRWAssets + (totalUSDAssets * exchangeRate), 'KRW')}</strong>
        </div>
      </div>
    </section>
  );

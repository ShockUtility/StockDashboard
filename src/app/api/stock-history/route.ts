...
export async function GET(request: Request) {
  ...
  console.log(`[주가 이력 조회] 시작 (종목: ${code}, 국가: ${country})`);
  ...
  return new Promise<NextResponse>((resolve) => {
    ...
    pyProcess.on('close', (codeStatus) => {
      if (codeStatus !== 0) {
        console.error(`[주가 이력 조회] 실패 (종료 코드: ${codeStatus})`);
      ...
      try {
        ...
        if (result.error) {
          console.error(`[주가 이력 조회] 내부 에러 응답: ${result.error}`);
          return resolve(NextResponse.json({ error: result.error }, { status: 400 }));
        }
        console.log(`[주가 이력 조회] 완료 (종목: ${code})`);
        return resolve(NextResponse.json(result));
      } catch (e: any) {
        console.error(`[주가 이력 조회] 파싱 에러: ${e.message}`);
